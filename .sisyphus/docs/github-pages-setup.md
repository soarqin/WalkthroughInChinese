...
- 自动化部署配置文件 `.github/workflows/deploy-github-pages.yml` 已合并至 `master` 分支。该文件定义了构建环境、依赖安装、VitePress 静态站点生成以及向 GitHub Pages 推送产物的逻辑。
...
- **手动触发**：
  ...
  4. 确认分支选择为 `master`，然后点击 「Run workflow」 按钮。
- **自动触发**：
  - 向 `master` 分支推送任何新的 commit 都会自动启动该工作流。
